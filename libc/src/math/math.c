...
double fabs(double x) {
    union { double d; uint64_t u; } un = { .d = x };
    un.u &= 0x7fffffffffffffffULL;
    return un.d;
}

float fabsf(float x) {
    union { float f; uint32_t u; } un = { .f = x };
    un.u &= 0x7fffffffU;
    return un.f;
}
...
float copysignf(float x, float y) {
    union { float f; uint32_t u; } ux = { .f = x }, uy = { .f = y };
...
double remquo(double x, double y, int *quo) {
    ...
    double q = round(x / y);
    if (quo) *quo = (int)((long long)q & 0x7fffffff);
    return x - q * y;
}
...
double ldexp(double x, int exp) {
    ...
    int e = exp;
    if (e > 0) {
        while (e >= 30) { factor *= (double)(1ULL << 30); e -= 30; }
        while (e > 0)  { factor *= 2.0; e--; }
        return x * factor;
    } else {
        while (e <= -30) { factor *= (1.0 / (double)(1ULL << 30)); e += 30; }
        while (e < 0)   { factor /= 2.0; e++; }
        return x * factor;
    }
}
...
float sqrtf(float x) {
    ...
    if (isnan(x) || isinf(x) || x == 0.0f) return x;
    float res;
...
double sin(double x) {
    ...
    if (isinf(x)) { errno = EDOM; return NAN; }
    if (fabs(x) > M_PI) x = remainder(x, 2.0 * M_PI);
    double res;
...
double sinh(double x) {
    if (isnan(x)) return NAN;
    if (fabs(x) > 710.0) return (x > 0) ? HUGE_VAL : -HUGE_VAL;
    double ex = exp(x);
    double emx = exp(-x);
    return (ex - emx) / 2.0;
}
...
double cosh(double x) {
    if (isnan(x)) return NAN;
    if (fabs(x) > 710.0) return HUGE_VAL;
    double ex = exp(x);
    double emx = exp(-x);
    return (ex + emx) / 2.0;
}
