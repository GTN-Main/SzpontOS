...
int pthread_mutex_timedlock(pthread_mutex_t *mutex, const struct timespec *abstime) {
    ...
    int c = __sync_val_compare_and_swap(&mutex->lock, 0, 1);
    if (c != 0) {
        if (c != 2) c = __sync_lock_test_and_set(&mutex->lock, 2);
        while (c != 0) {
...
int pthread_mutex_unlock(pthread_mutex_t *mutex) {
    ...
    if (mutex->type == PTHREAD_MUTEX_RECURSIVE) {
        pthread_t me = pthread_self();
        if (mutex->owner != (int)me) return EPERM;
        mutex->count--;
...
int pthread_mutexattr_settype(pthread_mutexattr_t *attr, int type) {
    if (!attr || (type != PTHREAD_MUTEX_NORMAL && type != PTHREAD_MUTEX_RECURSIVE && type != PTHREAD_MUTEX_ERRORCHECK)) return EINVAL;
    attr->type = type;
    return 0;
}
...
int pthread_rwlock_destroy(pthread_rwlock_t *rwlock) {
    if (!rwlock) return EINVAL;
    if (rwlock->readers > 0 || rwlock->writers > 0) return EBUSY;
    return 0;
}
...
int pthread_rwlock_tryrdlock(pthread_rwlock_t *rwlock) {
    if (!rwlock) return EINVAL;
    if (rwlock->writers > 0) return EBUSY;
    __sync_fetch_and_add(&rwlock->readers, 1);
    if (rwlock->writers > 0) {
        __sync_fetch_and_sub(&rwlock->readers, 1);
        return EBUSY;
...
int pthread_spin_trylock(pthread_spinlock_t *lock) {
    if (!lock) return EINVAL;
    if (__sync_lock_test_and_set(lock, 1)) return EBUSY;
    return 0;
}
...
int pthread_barrier_destroy(pthread_barrier_t *barrier) {
    ...
    if (barrier->count > 0) return EBUSY;
    return 0;
}
