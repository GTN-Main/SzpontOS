#!/usr/bin/env python3
"""QEMU regression test: runs /bin/corecheck via QMP-injected PS/2 scancodes."""
import argparse
import json
import os
import pathlib
...
p = argparse.ArgumentParser(description='QEMU PS/2 and memory regression test')
p.add_argument('--usb-image', action='store_true', help='Boot via USB disk instead of IDE/CD')
p.add_argument('--poll-only', action='store_true', help='Route PS/2 keyboard IRQ away to test polling')
