...
int main(int argc, char *argv[]) {
    ...
    for (int a = 1; a < argc; a++) {
        if (strcmp(argv[a], "-h") == 0 || strcmp(argv[a], "--help") == 0) {
            ...
            printf("  -d <num>   Delay in milliseconds between frames (default: 16)\n");
            return 0;
    ...
    while (max_frames < 0 || frame_count < max_frames) {
        ...
        for (j = 0.0f; j < 6.28f; j += 0.07f) {
            ...
            for (i = 0.0f; i < 6.28f; i += 0.02f) {
                ...
                int o = x + 80 * y;
                int N = (int)(8 * ((sin_j * sin_A - sin_i * cos_j * cos_A) * cos_B - sin_i * cos_j * sin_A -
                                   sin_j * cos_A - cos_i * cos_j * sin_B));
