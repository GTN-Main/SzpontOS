...
int main(int argc, char *argv[]) {
    ...
    move_to(4, 5);
    printf("%sOS:%s       SzpontOS v0.1.0", BOLD, RESET);
    move_to(5, 5);
    printf("%sArch:%s     x86_64 Higher-Half", BOLD, RESET);
    move_to(6, 5);
    printf("%sTerminal:%s Framebuffer Console", BOLD, RESET);
    move_to(7, 5);
    printf("%sGeometry:%s %d cols x %d rows", BOLD, RESET, cols, rows);
    move_to(8, 5);
    printf("%sResolution:%s%dx%d px", BOLD, RESET, ws.ws_xpixel, ws.ws_ypixel);
    move_to(9, 5);
    printf("%sDriver:%s   Virtio-VGA / Limine FB", BOLD, RESET);
    move_to(10, 5);
    printf("%sStatus:%s   %s● OK (Active)%s", BOLD, RESET, FG_GREEN, RESET);

    /* 3. Color Palette Test Box */
    draw_box(3, 42, 35, 9, "ANSI 16 Colors", FG_YELLOW);
    move_to(5, 44);
    printf("Normal: %s██%s%s██%s%s██%s%s██%s%s██%s%s██%s%s██%s%s██%s", FG_BLACK, RESET, FG_RED, RESET, FG_GREEN, RESET,
           FG_YELLOW, RESET, FG_BLUE, RESET, FG_MAGENTA, RESET, FG_CYAN, RESET, FG_WHITE, RESET);
    move_to(7, 44);
    printf("Bright: "
           "\033[90m██\033[0m\033[91m██\033[0m\033[92m██\033[0m\033[93m██\033[0m\033[94m██\033[0m\033[95m██\033[0m\033["
           "96m██\033[0m\033[97m██\033[0m");
    ...
    printf("Arrows: ↑ ↓ → ← • ° ± ≤ ≥");
    move_to(18, 5);
    printf("Styles: %sBold%s %sUnderline%s %sReverse%s", BOLD, RESET, UNDERLINE, RESET, REVERSE, RESET);

    /* 5. Interactive Dialog Simulation */
    draw_double_box(13, 42, 35, 8, "Dialog / Notification", FG_CYAN);
    move_to(15, 45);
    printf("%sTUI Rendering Verified!%s", BOLD, RESET);
    move_to(16, 45);
    printf("All ANSI & UTF-8 boxes OK.");
    move_to(18, 50);
    printf("%s[ OK / ENTER ]%s", REVERSE FG_WHITE, RESET);
