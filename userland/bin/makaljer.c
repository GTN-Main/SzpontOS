/*
 * SzpontOS — Native X11 Artwork Viewer: Makaljer
 * (C) Copyright by Szpont Industries. All rights reserved.
 *
 * Standalone process displaying Makaljer PNG artwork in an independent X11 window.
 */
...
static loaded_image_t load_image_file(const char *path1, const char *path2, const char *path3) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (raw) {
            printf("[makaljer] Loaded artwork '%s' (%dx%d, %d channels)\n",
                   paths[i], w, h, ch);
            break;
        }
    }
    ...
    if (!raw) {
        printf("[makaljer] Warning: Artwork not found (%s)\n", path1);
        return img;
    }
...
int main(int argc, char *argv[]) {
    ...
    if (!dpy) {
        fprintf(stderr, "[makaljer] Cannot open display '%s'\n", disp_name);
        return 1;
    }
    ...
    loaded_image_t img = load_image_file("/usr/share/artwork/makaljer.png",
    ...
    Window win = XCreateSimpleWindow(dpy, root, win_x, win_y,
                                    WIN_WIDTH, WIN_HEIGHT, 0, pink_color, card_bg);

    XStoreName(dpy, win, "Makaljer (PNG Artwork)");
