...
void net_init(void) {
    ...
    if (!nic)
        nic = pci_find_device(E1000_VENDOR_INTEL, E1000_DEV_82545EM);
    if (!nic)
        nic = pci_find_device(E1000_VENDOR_INTEL, E1000_DEV_82574L);
    if (!nic)
        nic = pci_find_device(E1000_VENDOR_INTEL, 0x1004);
    if (!nic)
        nic = pci_find_device(E1000_VENDOR_INTEL, 0x1539);

    if (nic) {
        e1000_init(nic);
    }

    /* Scan PCI for Realtek RTL8139 Fast Ethernet Cards */
    rtl8139_init();

    if (!nic && !netif_find_by_name("eth1")) {
