...
static size_t procfs_gen_cpuinfo(char *buf, size_t max_len) {
    ...
    if (max_ext >= 0x80000004) {
        cpuid(0x80000002, 0, (uint32_t *)&brand[0], (uint32_t *)&brand[4], (uint32_t *)&brand[8], (uint32_t *)&brand[12]);
        cpuid(0x80000003, 0, (uint32_t *)&brand[16], (uint32_t *)&brand[20], (uint32_t *)&brand[24], (uint32_t *)&brand[28]);
        cpuid(0x80000004, 0, (uint32_t *)&brand[32], (uint32_t *)&brand[36], (uint32_t *)&brand[40], (uint32_t *)&brand[44]);
    } else {
    ...
    return ksnprintf(buf, max_len,
        ...
        "cache size\t: 4096 KB\n"
        "fpu\t\t: yes\n"
        "flags\t\t: fpu vme de pse tsc msr pae mce cx8 apic sep mtrr pge mca cmov pat pse36 clflush mmx fxsr sse sse2 sse3 ssse3 sse4_1 sse4_2 avx\n"
        "bogomips\t: 4800.00\n"
        "clflush size\t: 64\n"
...
static size_t procfs_gen_version(char *buf, size_t max_len) {
    return ksnprintf(buf, max_len,
        "SzpontOS version 0.1.0 (x86_64 Higher-Half) (gcc 13.2.0, Limine 8.x) #1 PREEMPT\n");
}
...
static size_t procfs_gen_mounts(char *buf, size_t max_len) {
    ...
    for (size_t i = 0; i < count && pos < max_len; i++) {
        ...
        if (strcmp(mounts[i].path, "/dev") == 0) fs_type = "devfs";
        else if (strcmp(mounts[i].path, "/proc") == 0) fs_type = "procfs";
        else if (strcmp(mounts[i].path, "/mnt") == 0) fs_type = "ext2";
...
static void procfs_module_formatter(module_t *mod, void *arg) {
    ...
    if (ctx->offset >= ctx->max_len) return;

    const char *state_str = "Live";
    if (mod->state == MODULE_STATE_COMING) state_str = "Loading";
    else if (mod->state == MODULE_STATE_GOING) state_str = "Unloading";

    ctx->offset += ksnprintf(ctx->buf + ctx->offset, ctx->max_len - ctx->offset,
        "%s %lu %d - %s 0x%lx\n",
        mod->name, mod->size, mod->refcnt, state_str, mod->base_addr);
}
...
static size_t procfs_gen_proc_status(process_t *proc, char *buf, size_t max_len) {
    ...
    if (proc->status == PROCESS_ZOMBIE) state_str = "Z (zombie)";
    else if (proc->status == PROCESS_DEAD) state_str = "X (dead)";
    ...
    return ksnprintf(buf, max_len,
        ...
        "VmSize:\t    8192 kB\n"
        "VmRSS:\t      64 kB\n",
        proc->name, state_str, proc->pid, proc->pid, proc->ppid,
        proc->uid, proc->euid, proc->uid, proc->euid,
        proc->gid, proc->egid, proc->gid, proc->egid,
        MAX_FD, proc->gid);
}
...
static size_t procfs_gen_proc_stat(process_t *proc, char *buf, size_t max_len) {
    char state_char = 'R';
    if (proc->status == PROCESS_ZOMBIE) state_char = 'Z';
    else if (proc->status == PROCESS_DEAD) state_char = 'X';
    ...
    return ksnprintf(buf, max_len,
        "%d (%s) %c %d %d %d 0 -1 4194304 0 0 0 0 10 5 0 0 20 0 1 0 100 8388608 16 18446744073709551615 4194304 4362240 140737218686976 0 0 0 0 0 0 0 0 0 17 0 0 0 0 0 0\n",
...
static ssize_t procfs_file_read(vfs_node_t *node, off_t offset, size_t size, void *buffer) {
    ...
    switch (data->type) {
        case PROCFS_TYPE_MEMINFO:     content_len = procfs_gen_meminfo(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_CPUINFO:     content_len = procfs_gen_cpuinfo(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_VERSION:     content_len = procfs_gen_version(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_UPTIME:      content_len = procfs_gen_uptime(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_STAT:        content_len = procfs_gen_stat(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_LOADAVG:     content_len = procfs_gen_loadavg(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_MOUNTS:      content_len = procfs_gen_mounts(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_FILESYSTEMS: content_len = procfs_gen_filesystems(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_CMDLINE:     content_len = procfs_gen_cmdline(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_DEVICES:     content_len = procfs_gen_devices(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_SELF:        content_len = procfs_gen_self(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_MODULES:     content_len = procfs_gen_modules(tmp_buf, sizeof(tmp_buf)); break;
        case PROCFS_TYPE_SYSCTL:      content_len = procfs_gen_sysctl(tmp_buf, sizeof(tmp_buf)); break;
        default: {
            process_t *proc = process_get_by_pid(data->pid);
            if (!proc) {
                if (data->pid == 0) proc = sched_get_current_process();
            }
            if (!proc) return 0;

            switch (data->type) {
                case PROCFS_PID_STATUS:  content_len = procfs_gen_proc_status(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_CMDLINE: content_len = procfs_gen_proc_cmdline(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_COMM:    content_len = procfs_gen_proc_comm(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_CWD:     content_len = procfs_gen_proc_cwd(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_EXE:     content_len = procfs_gen_proc_exe(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_STAT:    content_len = procfs_gen_proc_stat(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_STATM:   content_len = procfs_gen_proc_statm(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_MAPS:    content_len = procfs_gen_proc_maps(proc, tmp_buf, sizeof(tmp_buf)); break;
                case PROCFS_PID_ENVIRON: content_len = procfs_gen_proc_environ(proc, tmp_buf, sizeof(tmp_buf)); break;
                default: break;
            }
            break;
        }
...
static const char *g_proc_pid_files[] = {
    "status",
    "cmdline",
    "comm",
    "cwd",
...
static struct vfs_dirent *procfs_pid_dir_readdir(vfs_node_t *node, uint32_t index) {
    ...
    if (index >= PROC_PID_FILE_COUNT) return NULL;
...
static struct vfs_node *procfs_pid_dir_finddir(vfs_node_t *node, const char *name) {
    ...
    if (strcmp(name, "status") == 0)  return procfs_create_file_node("status", PROCFS_PID_STATUS, pid);
    if (strcmp(name, "cmdline") == 0) return procfs_create_file_node("cmdline", PROCFS_PID_CMDLINE, pid);
    if (strcmp(name, "comm") == 0)    return procfs_create_file_node("comm", PROCFS_PID_COMM, pid);
    if (strcmp(name, "cwd") == 0)     return procfs_create_file_node("cwd", PROCFS_PID_CWD, pid);
    if (strcmp(name, "exe") == 0)     return procfs_create_file_node("exe", PROCFS_PID_EXE, pid);
    if (strcmp(name, "stat") == 0)    return procfs_create_file_node("stat", PROCFS_PID_STAT, pid);
    if (strcmp(name, "statm") == 0)   return procfs_create_file_node("statm", PROCFS_PID_STATM, pid);
    if (strcmp(name, "maps") == 0)    return procfs_create_file_node("maps", PROCFS_PID_MAPS, pid);
    if (strcmp(name, "environ") == 0) return procfs_create_file_node("environ", PROCFS_PID_ENVIRON, pid);
...
static const struct {
    const char *name;
    uint32_t type;
} g_proc_static_entries[] = {
    { "meminfo",     PROCFS_TYPE_MEMINFO },
    { "cpuinfo",     PROCFS_TYPE_CPUINFO },
    { "version",     PROCFS_TYPE_VERSION },
    { "uptime",      PROCFS_TYPE_UPTIME },
    { "stat",        PROCFS_TYPE_STAT },
    { "loadavg",     PROCFS_TYPE_LOADAVG },
    { "mounts",      PROCFS_TYPE_MOUNTS },
    { "filesystems", PROCFS_TYPE_FILESYSTEMS },
    { "cmdline",     PROCFS_TYPE_CMDLINE },
    { "devices",     PROCFS_TYPE_DEVICES },
    { "self",        PROCFS_TYPE_SELF },
    { "modules",     PROCFS_TYPE_MODULES },
    { "dmesg",       PROCFS_TYPE_DMESG },
    { "kmsg",        PROCFS_TYPE_DMESG },
    { "sysctl",      PROCFS_TYPE_SYSCTL }
};
