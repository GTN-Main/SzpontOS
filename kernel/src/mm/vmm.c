...
static page_table_t *get_next_level(page_table_t *current, size_t index, int level, bool allocate, uint64_t flags) {
    uint64_t entry = current->entries[index];

    if (entry & VMM_FLAG_PRESENT) {
        if (entry & (1 << 7)) {
            /* Huge page entry (2 MiB in PD or 1 GiB in PDPT) */
            if (!allocate) {
                return NULL;
            }
            ...
            if (level == 2) {
                /* Splitting 1 GiB huge page in PDPT into 512 2 MiB huge pages in PD */
                uintptr_t base_phys = entry & 0x000FFFFFFC000000ULL;
                uint64_t page_flags = entry & 0xFFF; /* Keeps bit 7 (PS) */

                for (size_t p = 0; p < 512; p++) {
                    new_table_virt->entries[p] = (base_phys + p * 0x200000ULL) | page_flags;
                }
            } else {
                /* Splitting 2 MiB huge page in PD into 512 4 KiB pages in PT */
                uintptr_t base_phys = entry & 0x000FFFFFFFF00000ULL;
                uint64_t page_flags = (entry & 0xFFF) & ~(1ULL << 7); /* Clears bit 7 (PS) */

                for (size_t p = 0; p < 512; p++) {
                    new_table_virt->entries[p] = (base_phys + p * PAGE_SIZE) | page_flags;
                }
            }

            current->entries[index] = new_table_phys | VMM_FLAG_PRESENT | VMM_FLAG_WRITABLE | (entry & (VMM_FLAG_USER | flags));
...
bool vmm_map_page(pagemap_t *map, uintptr_t virt, uintptr_t phys, uint64_t flags) {
    ...
    page_table_t *pdpt = get_next_level(pml4, pml4_index(virt), 3, true, flags & VMM_FLAG_USER);
    if (!pdpt) {
        spinlock_release(&g_vmm_lock);
        return false;
    }

    page_table_t *pd = get_next_level(pdpt, pdpt_index(virt), 2, true, flags & VMM_FLAG_USER);
    if (!pd) {
        spinlock_release(&g_vmm_lock);
        return false;
    }

    page_table_t *pt = get_next_level(pd, pd_index(virt), 1, true, flags & VMM_FLAG_USER);
    if (!pt) {
        spinlock_release(&g_vmm_lock);
        return false;
    }
...
bool vmm_unmap_page(pagemap_t *map, uintptr_t virt) {
    ...
    page_table_t *pml4 = map->pml4_virt;
    page_table_t *pdpt = get_next_level(pml4, pml4_index(virt), 3, false, 0);
    if (!pdpt) {
        spinlock_release(&g_vmm_lock);
        return false;
    }

    page_table_t *pd = get_next_level(pdpt, pdpt_index(virt), 2, false, 0);
    if (!pd) {
        spinlock_release(&g_vmm_lock);
        return false;
    }

    page_table_t *pt = get_next_level(pd, pd_index(virt), 1, false, 0);
    if (!pt) {
        spinlock_release(&g_vmm_lock);
        return false;
    }
...
uintptr_t vmm_virt_to_phys(pagemap_t *map, uintptr_t virt) {
    ...
    page_table_t *pml4 = map->pml4_virt;
    uint64_t pml4e = pml4->entries[pml4_index(virt)];
    if (!(pml4e & VMM_FLAG_PRESENT))
        return 0;

    page_table_t *pdpt = (page_table_t *)PHYS_TO_VIRT(pml4e & PHYS_ADDR_MASK);
    uint64_t pdpte = pdpt->entries[pdpt_index(virt)];
    if (!(pdpte & VMM_FLAG_PRESENT))
        return 0;

    if (pdpte & (1 << 7)) {
        /* 1 GiB Huge Page */
        return (pdpte & 0x000FFFFFFC000000ULL) | (virt & 0x3FFFFFFFULL);
    }

    page_table_t *pd = (page_table_t *)PHYS_TO_VIRT(pdpte & PHYS_ADDR_MASK);
    uint64_t pde = pd->entries[pd_index(virt)];
    if (!(pde & VMM_FLAG_PRESENT))
        return 0;

    if (pde & (1 << 7)) {
        /* 2 MiB Huge Page */
        return (pde & 0x000FFFFFFFF00000ULL) | (virt & 0x1FFFFFULL);
    }

    page_table_t *pt = (page_table_t *)PHYS_TO_VIRT(pde & PHYS_ADDR_MASK);
    uint64_t pte = pt->entries[pt_index(virt)];
    if (!(pte & VMM_FLAG_PRESENT))
        return 0;

    return (pte & PHYS_ADDR_MASK) | (virt & 0xFFFULL);
}
