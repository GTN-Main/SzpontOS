...
static uint8_t g_raw_scancode_buf[KEYBOARD_BUFFER_SIZE];
static volatile size_t g_raw_read_ptr = 0;
static volatile size_t g_raw_write_ptr = 0;
...
static int kbd_device_command(bool aux, uint8_t cmd) {
    for (int retry = 0; retry < 2; retry++) {
        ...
        kbd_write_data(cmd);
        for (unsigned elapsed = 0; elapsed < 20000; elapsed += 50) {
            uint8_t status = kbd_read_status();
            if (status != 0xFF && (status & KBDS_OBF)) {
                uint8_t data = kbd_read_data();
...
void keyboard_push_raw_scancode(uint8_t scancode) {
    size_t w = __atomic_load_n(&g_raw_write_ptr, __ATOMIC_RELAXED);
    size_t r = __atomic_load_n(&g_raw_read_ptr, __ATOMIC_ACQUIRE);
    size_t next_write = (w + 1) % KEYBOARD_BUFFER_SIZE;
    if (next_write != r) {
        g_raw_scancode_buf[w] = scancode;
        __atomic_store_n(&g_raw_write_ptr, next_write, __ATOMIC_RELEASE);
    }
}

bool keyboard_has_raw_scancode(void) {
    keyboard_poll_hardware();
    return (__atomic_load_n(&g_raw_read_ptr, __ATOMIC_ACQUIRE) !=
            __atomic_load_n(&g_raw_write_ptr, __ATOMIC_ACQUIRE));
}

uint8_t keyboard_get_raw_scancode(void) {
    if (!keyboard_has_raw_scancode())
        return 0;
    size_t r = __atomic_load_n(&g_raw_read_ptr, __ATOMIC_RELAXED);
    uint8_t sc = g_raw_scancode_buf[r];
    size_t next_read = (r + 1) % KEYBOARD_BUFFER_SIZE;
    __atomic_store_n(&g_raw_read_ptr, next_read, __ATOMIC_RELEASE);
    return sc;
}
...
static void kbd_set_leds_internal(bool numlock, bool capslock, bool scrolllock, bool need_lock) {
    g_num_lock = numlock;
    ...
    uint64_t flags = 0;
    if (need_lock) {
        flags = keyboard_controller_acquire();
    }
...
void keyboard_set_leds(bool numlock, bool capslock, bool scrolllock) {
    kbd_set_leds_internal(numlock, capslock, scrolllock, true);
}
...
static void process_scancode(uint8_t scancode) {
    ...
    if (g_extended) {
        g_extended = false;
        keyboard_push_raw_scancode(scancode);
        ...
        case 0x35: ext_evcode = 98;  break; /* Keypad / -> KEY_KPSLASH */
        case 0x1D: ext_evcode = 97;  break; /* Right Ctrl -> KEY_RIGHTCTRL */
        case 0x38: ext_evcode = 100; break; /* Right Alt -> KEY_RIGHTALT */
        }
        if (ext_evcode) {
            evdev_push_key(ext_evcode, !(scancode & 0x80));
        }

        /* Extended Key Release (Bit 7 Set) */
        if (scancode & 0x80) {
            uint8_t released = scancode & 0x7F;
            if (released == 0x1D) g_rctrl = false;
            else if (released == 0x38) g_ralt = false;
            return;
        }
        ...
        case 0x53: push_char_lockless(127); return;      /* Delete */
        case 0x1C: push_char_lockless('\r'); return;     /* Keypad Enter */
        case 0x35: push_char_lockless('/'); return;      /* Keypad / */
        case 0x1D: g_rctrl = true; return;               /* Right Ctrl */
        case 0x38: g_ralt = true; return;                /* Right Alt */
        default: return;
        }
    }
    ...
    if (scancode == 0x3A) { /* Caps Lock */
        ...
        kbd_set_leds_internal(g_num_lock, g_caps_lock, g_scroll_lock, false);
        return;
    }
    if (scancode == 0x45) { /* Num Lock */
...
void keyboard_flush(void) {
    size_t w = __atomic_load_n(&g_kb_write_ptr, __ATOMIC_RELAXED);
    __atomic_store_n(&g_kb_read_ptr, w, __ATOMIC_RELEASE);
}
