 ...
 * -1 = Auto-detect (convert Cartesian Y to screen coordinates)
 *  0 = Off (Preserve raw PS/2 Cartesian Y)
 *  1 = On  (Invert Cartesian Y displacement to standard screen coordinates)
 */
static int g_ps2_invert_y = 1;

static __attribute__((unused)) bool is_hypervisor(void) {
...
static bool should_invert_y(void) {
    ...
    if (g_ps2_invert_y == 0)
        return false;
    /* Convert Cartesian PS/2 hardware Y to standard screen coordinates */
    return true;
}
...
static uint64_t g_last_packet_tick = 0;

void ps2_mouse_handle_byte(uint8_t byte) {
    uint64_t now = pit_get_ticks();
    if (g_packet_idx > 0 && (now - g_last_packet_tick) > 50) {
        /* Timeout (>50ms) between bytes of partial packet: drop and resync */
        g_packet_idx = 0;
    }
    g_last_packet_tick = now;
