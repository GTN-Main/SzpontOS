...
static uint64_t datetime_to_epoch(const rtc_time_t *dt) {
    ...
    uint64_t total_seconds = total_days * 86400ULL +
...
void rtc_read_datetime(rtc_time_t *dt) {
    ...
    while (1) {
        ...
        if (last.second == curr.second && last.minute == curr.minute &&
            last.hour == curr.hour && last.day == curr.day &&
            last.month == curr.month && last.year == curr.year) {
            break;
        }
    }
...
static uint64_t g_tsc_freq_hz = 2400000000ULL; /* Default 2.4 GHz fallback */
...
#else
    return pit_get_ticks() * 1000000ULL;
#endif
}
...
static void calibrate_tsc(void) {
    ...
    if (timeout > 0 && end_tsc > start_tsc) {
        ...
        g_tsc_freq_hz = diff * 100ULL; /* 10 ms * 100 = 1 sec */
    } else {
...
void rtc_init(void) {
    ...
    klog_info("RTC initialized: %04u-%02u-%02u %02u:%02u:%02u UTC (Epoch: %llu)",
              dt.year, dt.month, dt.day, dt.hour, dt.minute, dt.second,
              (unsigned long long)g_rtc_boot_epoch_sec);
